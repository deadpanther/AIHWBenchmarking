...
from aihwkit.nn import AnalogLinear, AnalogConv1d, AnalogConv2d, AnalogConv3d, AnalogSequential
...
from aihwkit.optim import AnalogSGD
import aihwkit.nn.modules.container
...
def create_analog_network1():
    """Return a LeNet5 inspired analog model."""
    model = AnalogSequential(            
            AnalogConv2d(in_channels=3, out_channels=32, kernel_size=5, stride=1, rpu_config=RPU_CONFIG), 
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=2),
            AnalogConv2d(in_channels=32, out_channels=64, kernel_size=3, stride=1, rpu_config=RPU_CONFIG),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            AnalogConv2d(in_channels=64, out_channels=128, kernel_size=3, stride=1, rpu_config=RPU_CONFIG),
            nn.BatchNorm2d(128),
            nn.ReLU(), 
            AnalogConv2d(in_channels=128, out_channels=256, kernel_size=3, stride=1, rpu_config=RPU_CONFIG),
            nn.BatchNorm2d(256),
            nn.ReLU(),     
            AnalogConv2d(in_channels=256, out_channels=256, kernel_size=3, stride=1,padding=1, rpu_config=RPU_CONFIG),
            nn.BatchNorm2d(256),
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=2),
            AnalogConv2d(in_channels=256, out_channels=512, kernel_size=3, stride=1, rpu_config=RPU_CONFIG),
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.Flatten(),
            AnalogLinear(in_features=2048, out_features=512, rpu_config=RPU_CONFIG),
            nn.ReLU(),  
...
def create_analog_network():
    ...
    model = AnalogSequential(
            AnalogConv2d(in_channels=3, out_channels=channel[0], kernel_size=5, stride=1,rpu_config=RPU_CONFIG),
            nn.Tanh(),
...
def main():
    ...
    os.environ['MASTER_ADDR'] = '10.32.35.104'
    os.environ['MASTER_PORT'] = '8006'
...
def main_worker(local_rank, nprocs, args):
    ...
    optimizer = AnalogSGD(model.parameters(), args.lr)
    ...
    np.save("train_accuracy_4analog.npy", train_accuracy_values)
    np.save("validation_accuracy_4analog.npy",validation_accuracy_values)
    np.save("total_time_per_epoch_4analog.npy", total_time_array/60)
